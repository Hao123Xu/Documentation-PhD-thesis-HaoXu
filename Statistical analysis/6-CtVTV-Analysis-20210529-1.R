...
##############################################################
##
CtVTV=read.csv("D:/Desktop/Chapter7/2021_Jan_Quantitative New-3/1-BVTV-CtVTV-TbVTV/CtV_TV-analysis-2.csv")

## range=2
par( mar=c(5, 5, 3, 0.5))
boxplot(CtVTV[,1],CtVTV[,2],CtVTV[,3],CtVTV[,4],CtVTV[,5],CtVTV[,6],CtVTV[,7],CtVTV[,8], ylab=expression(bold("Ct.V/TV")), main="Cortical bone volume fractions", names=c("T1P","T1A","T1S","T1C","T2P","T2A","T2S","T2C"), xlab = expression(bold("Group")),cex.lab=2, cex.axis=1.5, cex.main=2,col=terrain.colors(8),boxlwd = 1.5, whisklty = 2, staplelwd = 1.5,outcex = 2, medlwd=1.5, medcol = "red", range=1.5)
box(lwd=2)
par(font.axis = 2)
