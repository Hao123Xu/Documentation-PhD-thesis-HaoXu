##
BVTV=read.csv("D:/Desktop/Chapter7/2021_Jan_Quantitative New-3/9-Vessel/FD-vessel-1.csv")

## 
par( mar=c(5, 5, 3, 0.5))
boxplot(BVTV[,1],BVTV[,2],BVTV[,3],BVTV[,4],BVTV[,5],BVTV[,6],BVTV[,7],BVTV[,8], ylab=expression(bold("FD")), main="Fractal dimension of vessel", names=c("T1P","T1A","T1S","T1C","T2P","T2A","T2S","T2C"), xlab = expression(bold("Group")),cex.lab=2, cex.axis=1.5, cex.main=2,col=terrain.colors(8),boxlwd = 1.5, whisklty = 2, staplelwd = 1.5,outcex = 2, medlwd=1.5, medcol = "red", range=2)
box(lwd=2)
par(font.axis = 2)
