##
BVTV=read.csv("D:/Desktop/Chapter7/2021_Jan_Quantitative New-3/9-Vessel/VVTV-1.csv")

## 
par( mar=c(5, 5, 3, 0.5))
boxplot(BVTV[,1],BVTV[,2],BVTV[,3],BVTV[,4],BVTV[,5],BVTV[,6],BVTV[,7],BVTV[,8], ylab=expression(bold("VV/TV")), main="Vessel volume fractions", names=c("T1P","T1A","T1S","T1C","T2P","T2A","T2S","T2C"), xlab = expression(bold("Group")),cex.lab=2, cex.axis=1.5, cex.main=2,col=terrain.colors(8),boxlwd = 1.5, whisklty = 2, staplelwd = 1.5,outcex = 2, medlwd=1.5, medcol = "red", range=1.5)
box(lwd=2)
par(font.axis = 2)
...
A <- factor(c(rep("group1",10),rep("group2",13),rep("group3",8),rep("group4",11)))
DATA <- data.frame(BVTVnew,A)
bartlett.test(BVTVnew~A,data=DATA)

# T2
BVTVnew=c(BVTVnew5,BVTVnew6,BVTVnew7,BVTVnew8)
A <- factor(c(rep("group5",7),rep("group6",7),rep("group7",7),rep("group8",5)))
DATA <- data.frame(BVTVnew,A)
bartlett.test(BVTVnew~A,data=DATA)
...
A <- factor(c(rep("T1P",10),rep("T1A",13),rep("T1S",8),rep("T1C",11)))
...
A <- factor(c(rep("T2P",7),rep("T2A",7),rep("T2S",7),rep("T2C",5)))
DATA <- data.frame(BTH,A)
a.aov <- aov(BTH~A,data=DATA) 
summary(a.aov)
