function imgdata=read_mhd_new(filename,filenumber)
...
display('Tibi-s version of read_mhd');
info = mhareadheader(filename);
[path name extension] = fileparts(filename);
% info.Offset=[0,0,95]*(filenumber-1)+15;
info.Offset=[0,0,112]*(filenumber-1);
% info.Offset=[0,0,320]*(filenumber-1);
% info.Offset=[0,0,112]*(filenumber-1)+3000;
...
if (ndims == 1)
    data = read_raw([ path  filesep  info.DataFile ], info.Dimensions,info.DataType,'native',info.Offset,ndims);
...
elseif (ndims == 3)
    ...
    [datax datay dataz] = read_raw([ path  filesep  info.DataFile], info.Dimensions,info.DataType,'native',info.Offset,ndims);
...
function [rawData rdy rdz] =read_raw(filename,imSize,type,endian,offset,ndims)
% Reads a raw file
...
% Output: image
% offse
skip=imSize(1)*imSize(2)*4*offset(3);
% skip=4*offset;
rdy=[];
rdz=[];
...
if (fid < 0)
    ...
    if (ndims == 1)
        ...
        if status == 0
            rawData = fread(fid,prod(imSize),[type '=>' type]);
            fclose(fid);
...
while(~readelementdatafile)
    ...
    switch(lower(type))
        ...
        %added by Tibi: elementnumberofchannels and '' (empty string)
        case 'elementnumberofchannels'
            info.ElementNumberOfChannels=data;
        case ''
            % do nothing
        otherwise
            info.(type)=data;
